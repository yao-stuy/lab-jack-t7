#!/bin/sh
# setup.sh — install build/runtime dependencies into a virtualenv.
# Run automatically by `viam module build` / `viam module reload` (meta.json -> build.setup)
# and again as the first step of build.sh.
cd `dirname $0`
